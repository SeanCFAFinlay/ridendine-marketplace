...
| `CONDITIONAL` | Implemented/verified locally but still requires staging/remote/owner evidence before full closure |
...
| IRR-003 | HIGH | Security | 15 | Scope every `createAdminClient` web query by session `customerId`; security tests | `apps/web/src/app/api/**/*.ts` (cart, favorites, reviews, addresses, etc.) | Engine helpers, DB | Per-route security tests + `pnpm typecheck` | No cross-customer data in tests | **DONE** | `cursor-phase15-2026-04-30`, `phase-b-exec-2026-05-01` | Phase B added ownership and service-role boundary tests (`apps/web/__tests__/security/ownership-guards.test.ts`, `apps/web/__tests__/security/service-role-boundary.test.ts`) with passing verification. |
...
| IRR-016 | LOW | Documentation | 1 | Generate `graphify-out/GRAPH_REPORT.md` or remove workspace rule | Repo root, user `CLAUDE.md` if applicable | OPTIONAL | N/A or doc-only | Decision recorded | **BLOCKED** | `phase-g-exec-2026-05-01` | Graphify rebuild repeatedly hangs in this workspace; documented as tooling blocker pending owner/tooling decision. |
...
| IRR-019 | MEDIUM | Driver flow | 8, 11 | Rate limit on driver location API (+ optional Redis later) | `apps/driver-app/.../location/route.ts`, `packages/utils` `rate-limiter.ts`, `location-client-time.ts` | utils | Vitest `driverLocation` preset + time guard | Authenticated: **429** when bucket exceeded; **401** unauth; **Retry-After** header | **CONDITIONAL** | `phase-e-exec-2026-05-01`, `phase-g-exec-2026-05-01` | Route-level wiring complete and tested; distributed provider still not configured in staging/production-like envs. |
...
| IRR-022 | HIGH | Business engine | 5, 6, 9 | Add `risk.engine.ts`; hook checkout/dispatch | `packages/engine/src/orchestrators/risk.engine.ts` (NEW), `packages/engine/src/index.ts`, callers | Engine | Vitest rules | Flagged high-risk orders blocked with reason | **DONE** | `cursor-phase5-2026-04-30`, `phase-c-exec-2026-05-01` | Risk engine implemented and checkout route hook verified with `RISK_BLOCKED` behavior and regression tests in Phase A/C evidence. |
...
| IRR-024 | LOW | Performance | 17 | Load test checkout (k6/Artillery) + SLO | Infra docs, staging | Phase 6+9 stable | Load test report | SLO numbers in `docs/` | **CONDITIONAL** | `phase-e-exec-2026-05-01`, `phase-g-exec-2026-05-01` | Load tooling and templates are complete; staged execution and signed report evidence are still required to close fully. |
...
**Last updated:** 2026-05-01 — Phase G release-doc pass aligned statuses for IRR-003, IRR-016, IRR-019, IRR-022, IRR-024 with current A-F execution evidence and remaining staging/tooling gates.
